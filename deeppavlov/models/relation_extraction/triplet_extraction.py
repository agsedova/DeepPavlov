...
class TripletExtractor(Component):
    ...
    def __call__(self, entity_substr_batch, entity_offsets_batch, tags_batch, probas_batch, sentences_offsets_batch,
        ...
        tags_batch = self.substitute_ner_tags(tags_batch)
    ...
    def substitute_ner_tags(self, tags_batch: List[List[str]]) -> List[List[str]]:
        """
        Replace the NER tags that are not in the list of allowed once with equivalents from the list.
        Args:
            List of Lists of NER tags.
        Return:
            Corrected List of Lists of NER tags.
        """
        tags_batch_corrected = []
        for tags in tags_batch:
            tags_corrected = []
            for ner_tag in tags:
                if ner_tag in self.ner_tags:
                    tags_corrected.append(ner_tag)
                else:
                    if ner_tag == "PERSON" and "PER" in self.ner_tags:
                        tags_corrected.append("PER")
                    elif ner_tag == "NORP" and "ORG" in self.ner_tags:
                        tags_corrected.append("ORG")
                    elif ner_tag in ["FACILITY", "ORGANIZATION", "GPE", "LOCATION"] and "LOC" in self.ner_tags:
                        tags_corrected.append("LOC")
                    elif ner_tag in ["DATE", "TIME"] and "TIME" in self.ner_tags:
